...
function randomWords(rnd, count) {
  const res = []
  for(let i=0; i<count; i++) {
    res.push(wordList[Math.floor(rnd() * wordList.length)])
  }
  return res
}

function makeWishlist(rnd, count, i) {
  const innerRnd = makeRandom(rnd())

  const name = ('Wish list ' + randomWords(innerRnd, 3 + Math.floor(rnd() * 5)).join(' ')).substr(0, 20)
  const products = [...new Array(count)].map((_, i) => makeProduct(innerRnd, i))
...
function makeProduct(rnd, i) {
  const innerRnd = makeRandom(rnd())
  const priceRnd = innerRnd()
  ...
  return {
    productId: ((parseInt(md5(title), 16) % 10000) * 100) + i,
    skuId: `${i}-${md5(title)}`,
    image: `https://images.codeite.net/image/jl/${imageId}?`,
    title,
...
app.get('/api/customer/v1/:userId/wish-lists/:listId', (req, res) => {
  setTimeout(() => {
    ...
    if (!list) res.status(404).json({err: 'List not found'})
    res.json(list)
  }, 500)
})

app.delete('/api/customer/v1/:userId/wish-lists/:listId/items/:skuId', (req, res) => {
  setTimeout(() => {
    const list = userWishLists[req.params.userId].find(x => x.id === req.params.listId)
    if (!list) res.status(404).json({err: 'List not found'})

    list.products = list.products.filter(x => x.skuId !== req.params.skuId)
...
app.get('/refresh/:seed', (req, res) => {
  refresh(req.params.seed)
  res.json(userWishLists)
})

app.get('/test', (req, res) => {
  const rand = makeRandom();
  res.json([1,2,3,4,5].map(x => `${x}+${rand()}`))
})

app.get('/test/:seed', (req, res) => {
  const rand = makeRandom(req.params.seed);
  res.json([1,2,3,4,5].map(x => `${x}+${rand()}`))
})
...
app.get('/image/:id', (req, res) => {
  ...
  const filePath = `./images/${id}-${size}px.jpg`
  ...
  readFile(filePath)
    ...
    .catch(e => {
      return fetch(`https://picsum.photos/${size}?image=${id}`)
        .then(r => r.buffer())
        .then(buffer => writeFile(filePath, buffer).then(buffer => {
        }))
...
function refresh(seed) {
  ...
  userWishLists['undefined'] = [0,1,2,49,50].map((count, i) => makeWishlist(rnd, count, i)),
  userWishLists['950a78eb1a4c498e8336fa4456de9c28'] = [50,49,10,1,0].map((count, i) => makeWishlist(rnd, count, i)),
  userWishLists['c97b395641dc475e95154fc41f3d2aa2'] = [50,49,10,1,0].map((count, i) => makeWishlist(rnd, count, i)),
  userWishLists['bill'] = []
}
...
const port = 3009
app.listen(3009, err => {
  if (err) console.error('Failed to listen on port 3010')
  else console.log(`The magic happens on port: ${3009}`)
})